<?php
    /**
     * Cette classe permet de gérer tout upload sur Webnel.
     * @package Webnel
     * @category Gestion des fichiers
    ...
    class Downloader {
         ...
         * @parap array    $data    The request's parameters.
         * @param string   $method  The request's method (self::GET or self::POST). Default value: self::GET.
        ...
        */
        protected function analyseHTTPRequest($HTTPResponse) {
            ...
            if(is_array($array_response) && count($array_response) >= 2) {
                ...
                throw new RuntimeException('The HTTP Response contains some errors. Headers and body must be divided by a blank line.');
            }
         ...
         * @param array     $data    The request's parameters.
         * @param string    $method  The request's method (self::GET or self::POST). Default value: self::GET.
         * @param array    &$step    An array used internally who store all sub-requests.
         * @return array             An array with the following keys:
         *   - data: the file's content;
         *   - contentType: the file's content-type;
         *   - HTTPCode: the HTTP Code returned by the server;
        ...
        protected function sockets($url, $data = array(), $method = self::GET, &$steps = array()) {
            ...
            if($socket = fsockopen($host, 80)) {
                ...
                if($infos['request_size'] == strlen($request)) {
                    ...
                    throw new RuntimeException('Error while sending request: some data was lost');
                }
         ...
         * @param string $erreur Une erreur éventuelle (variable affectée par référence).
         * @param array $donnees arrayleau de données à passer en paramètres au serveur distant.
         * @param string $methode Méthode d'envoi. Peut être self::GET (par défaut) ou self::POST.
