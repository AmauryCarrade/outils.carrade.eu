...
{
    // Multibyte str_split()
    private static function multibyte_str_split($string)
    {
        return preg_split('/(?<!^)(?!$)/u', $string);
    }

    // http://www.wiki.vg/Chat
    public static function parse_minecraft_colors($motd)
    {
        $inColorSequence = false;
        $openSpan = false;
        $coloredMotd = '';

        foreach (self::multibyte_str_split($motd) as $character)
        {
            if ($inColorSequence)
            {
